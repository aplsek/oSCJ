 ...
 * parameter specifying a specific physical address.  A full complement of
 * getter and setter methods allow the contents of the physical memory area to
 * be accessed through offsets from the base, interpreted as byte, short, int,
 * or long data values, and copied to/from arrays of byte, short, int, or long.
 *
 * By treating a value as itself an offset, a program can use a physical memory
 * area to contain "references" to other data values in the same area. A subregion
 * of a raw memory area can itself be defined as a raw memory area. The base
 * address and size, and any offset into a physical memory area, are long (64-bit)
 * values. Whether offset addresses the high or low order byte is based on the
 * value of the BYTE_ORDER static boolean variable in class RealtimeSystem. The
 * RawMemoryAccess class allows a real-time program to implement device drivers,
 * memory-mapped I/O, "flash" memory, battery-backed RAM, and similar low-level
 * software. 
 * 
 * 
 * Safety:
 * ------
 * A physical memory area cannot contain references to Java objects.
 * Such a capability would be unsafe (since it could be used to defeat Java's
 * type checking) and error-prone (since it is sensitive the the specific
...
public class RawMemoryAccess implements RawIntegralAccess {
	...
	/**
	 * Create a RawMemoryAccess object corresponding to the specified memory type and
	 * size.  
	 * 
	 * TODO: how the base address should be chosen? In J9 it is chosen automatically.
	 *
	...
	/**
	 * Create a RawMemoryAccess object corresponding to the specified memory type and
	 * size beginning at the specified physical address.
	 *
	 * @param 	type	The attribute(s) of the physical memory needed.  This is either one
	 * 					of the final type objects specified in PhysicalMemoryManager or an
	 * 					array of these objects.
	 *
	 * @param 	base	The physical address of the memory region to be mapped as a
	 * 					raw memory access area.
	 *
	 * @param 	size	The number of bytes to be allocated to the raw memory access area.
	 *
	 * @throws 	SecurityException
	 ...
	 */
	@SCJAllowed(LEVEL_0)          
	public static RawIntegralAccess createRmaInstance(RawMemoryName type, long base, long size) {
		return null; 
	}
	 ...
	 */
	public long map() {
		if (mappedBase != NOT_MAPPED) return mappedBase;

		// TODO: check if this will pass through the checker
		//		we are Level 0 and PhysicalMemoryManager is Level 1.
		mappedBase = PhysicalMemoryManager.map(type, physicalBase, physicalSize);
	 ...
	 *
	 * @param	offset	Offset of the byte to write
	...
	/**
	 * Get n bytes starting at the given offset in the mapped area, and assigns them
	 * into the byte array starting at position low.  While each of the loads from raw
	 * memory are atomic, there is no guarantee that the load of the entire array will
	 * be atomic.
	 *
	 * @param	offset	Offset of the bytes to read.
	 * @param	bytes	A destination byte array
	 * @param	low		An offset into the destination array to begin storing bytes
	 ...
	 * 			If low is negative or low+n is greater than or equal to bytes.length
	 *  throws javax.realtime.OffsetOutOfBoundsException,
         javax.realtime.SizeOutOfBoundsException
	 */
	@SCJAllowed(LEVEL_0)
	...
	/**
	 * Get the int at the given offset.  If the address indicated by the offset falls on a
	 * natural 4-byte boundary, then the value will be retrieved with an atomic load.
	 *
	 * @param	offset	Offset of the int to read
	...
	/**
	 * Get n ints starting at the given offset in the mapped area, and assigns them
	 * into the int array starting at position low.  While each of the loads from raw
	 * memory are atomic if naturally aligned, there is no guarantee that the load of
	 * the entire array will be atomic.
	 *
	 * @param	offset	Offset of the ints to read.
	 * @param	ints	A destination int array
	 * @param	low		An offset into the destination array to begin storing ints
	 ...
	 * 			If the offset points at an unmapped or otherwise invalid address
	 * @throws	ArrayIndexOutOfBoundsException
	 * 			If low is negative or low+n is greater than or equal to ints.length
	 * 
	 * /*       throws javax.realtime.OffsetOutOfBoundsException,
         javax.realtime.SizeOutOfBoundsException
	 * 
	...
	/**
	 * Get the long at the given offset.  The load is not guaranteed to be atomic, even if
	 * aligned on a natural 8-byte boundary.
	 *
	 * @param	offset	Offset of the long to read
	...
	/**
	 * Get n longs starting at the given offset in the mapped area, and assigns them
	 * into the long array starting at position low.  There is no guarantee that the load of
	 * the entire array or even a given element will be atomic, even if aligned on an 8-byte
	 * boundary.
	 *
	 * @param	offset	Offset of the longs to read.
	 * @param	longs	A destination long array
	 * @param	low		An offset into the destination array to begin storing longs
	 ...
	 * 			If the offset points at an unmapped or otherwise invalid address
	 * @throws	ArrayIndexOutOfBoundsException
	 * 			If low is negative or low+n is greater than or equal to longs.length
	 * /*       throws javax.realtime.OffsetOutOfBoundsException,
         javax.realtime.SizeOutOfBoundsException
	 * 
	 */
	...
	/**
	 * Get the short at the given offset.  If the address indicated by the offset falls on a
	 * natural 2-byte boundary, then the value will be retrieved with an atomic load.
	 *
	 * @param	offset	Offset of the short to read
	...
	/**
	 * Get n shorts starting at the given offset in the mapped area, and assigns them
	 * into the short array starting at position low.  While each of the loads from raw
	 * memory are atomic if naturally aligned, there is no guarantee that the load of
	 * the entire array will be atomic.
	 *
	 * @param	offset	Offset of the shorts to read.
	 * @param	shorts	A destination short array
	 * @param	low		An offset into the destination array to begin storing shorts
	 ...
	 * 			If the offset points at an unmapped or otherwise invalid address
	 * @throws	ArrayIndexOutOfBoundsException
	 * 			If low is negative or low+n is greater than or equal to shorts.length
	 * @throws javax.realtime.OffsetOutOfBoundsException,
         javax.realtime.SizeOutOfBoundsException
	 * 
	 */
	 ...
	 * memory are atomic, there is no guarantee that the store of the entire array will
	 * be atomic.
	 *
	 * @param	offset	Offset of the bytes to write.
	 * @param	bytes	A source byte array
	 * @param	low		An offset into the source array to begin reading bytes
	...
	/**
	 * Set the int at the given offset.  If the address indicated by the offset falls on a
	 * natural 4-byte boundary, then the value will be written with an atomic store.
	 *
	 * @param	offset	Offset of the int to write
	 * @param	v		Value to write.
	 *
	 * @throws	OffsetOutOfBoundsException
	 * 			If the offset is negative or falls beyond the size of the mapped area
	...
	/**
	 * Set the long at the given offset.  The store is not guaranteed to be atomic, even if
	 * aligned on a natural 8-byte boundary.
	 *
	 * @param	offset	Offset of the long to write
	 * @param	v		Value to write.
	 *
	 * @throws	OffsetOutOfBoundsException
	 * 			If the offset is negative or falls beyond the size of the mapped area
	 ...
	 */
	@SCJAllowed(LEVEL_0)
	public void setLong(long offset, long v) /* throws OffsetOutOfBoundsException */ {
	...
	/**
	 * Set the short at the given offset.  If the address indicated by the offset falls on a
	 * natural 2-byte boundary, then the value will be written with an atomic store.
	 *
	 * @param	offset	Offset of the short to write
	 * @param	v		Value to write.
	 *
	 * @throws	OffsetOutOfBoundsException
	 * 			If the offset is negative or falls beyond the size of the mapped area
	...
	/**
	 * Helper method - must not be protected as not user accessible
	 * but package private so RawMemoryFloatAccess can use it
	 * @param offset into RawMemoryAccess area
	...
	long checkOffset (long offset, long length) {
		...
		if (offset < 0 || offset >= physicalSize)
			throw new OffsetOutOfBoundsException(offset+":"+physicalSize);

		if (offset + length > mappedSize)
			throw new SizeOutOfBoundsException(offset+"+"+length+":"+mappedSize);
		 */
